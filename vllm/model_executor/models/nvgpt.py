...
class LoraLayer(nn.Module):
    def __init__(
        ...
        self.linear_in = ColumnParallelLinear(
            in_features, dim, bias=False, gather_output=True
        )
        self.linear_out = ColumnParallelLinear(
            dim, out_features, bias=False, gather_output=False
        )
...
class NVGPTMLP(torch.nn.Module):
    def __init__(
        ...
        super().__init__()
        self.dense_h_to_4h = ColumnParallelLinear(config.hidden_size, 2 * config.ffn_hidden_size, bias=config.bias,
                                                  gather_output=False)
        self.dense_4h_to_h = RowParallelLinear(config.ffn_hidden_size, config.hidden_size, bias=config.bias,
...
class NVGPTAttention(torch.nn.Module):
    def __init__(
        ...
        self.lora_layer = LoraLayer(in_features=self.hidden_size,
                                    out_features=3 * self.num_attention_heads * self.hidden_size_per_attention_head,
                                    dim=32)
    ...
    def forward(
        ...
        if input_metadata.customization_ids:
            ...
            for k, req_len in enumerate(lens):  # req_len: length of the current request
                ...
                if customization_id:
                    customization = input_metadata.customization_cache.get(customization_id)
                    if not customization:
                        logging.debug(f"customization {customization_id} not found. This code should never be reached")
                        break
                    lora_weights = customization.get_state_dict()
                    linear_in_weight = lora_weights[in_key]
                    linear_out_weight = lora_weights[out_key]
                    end = start + req_len
                    lora_qkv[start:end], _ = self.lora_layer(hidden_states[start:end], linear_in_weight,
                                                             linear_out_weight)
                    start += req_len
...
class NVGPTModel(torch.nn.Module):
    def __init__(self, config: NVGPTConfig):
        ...
        self.ptuning_embeddings = VocabParallelEmbedding(10, config.hidden_size)
    ...
    def forward(
        ...
        if len(vtokens) > 0:
            f = torch.tile(self.ptuning_embeddings(self.indices),
                           (len(vtokens) // self.ptuning_embeddings.num_embeddings, 1))
...
class NVGPTForCausalLM(torch.nn.Module):
    ...
    _column_parallel_weights = ["ptuning_embeddings.weight", "embedding.weight", "lm_head.weight",
                                "dense_h_to_4h.weight", "lora_layer.linear_in.weight"]
    _row_parallel_weights = ["dense.weight", "dense_4h_to_h.weight"]

    def map_name(self, name):
        return name.replace('model.language_model.encoder.', 'model.').replace("adapter_layer.lora_kqv_adapter.",
                                                                               "lora_layer.")
